...
def build_yfinance_company_info_table(company_info):
    ...
    longBusinessSummary VARCHAR(7000),\
...
def sql_query(query):
    query_results = []
    con = pymysql.connect( host="localhost", user="user1", passwd="user1", db="company_data" )
    with con:
        cur = con.cursor()
        print (query)
        cur.execute(query)
...
def write_to_table(these_columns):
    ...
    con = pymysql.connect( host="localhost", user="user1", passwd="user1", db="company_data", autocommit=True)
    with con:
        cur = con.cursor()
...
def process_company_info_json():
    ...
    myfd = open("/home/jfall/companyinfojson.txt","r")
    for line in myfd:
